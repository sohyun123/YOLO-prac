...
def load_image(path):
    ...
    img0 = cv.imread(path)
    # print(img0.shape)
    if img0.shape[0] >700 or img0.shape[1]>700:
        if img0.shape[0] >700:
            scale = 700/img0.shape[0]
            img0 = cv.resize(img0, dsize=(0, 0), fx=scale, fy=scale, interpolation=cv.INTER_LINEAR)
        elif img0.shape[1] > 700:
            scale = 700 / img0.shape[1]
            img0 = cv.resize(img0, dsize=(0, 0), fx=scale, fy=scale, interpolation=cv.INTER_LINEAR)
    # print(img0.shape)
